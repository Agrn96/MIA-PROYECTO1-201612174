...
void obmkusr::mostrardatos(obmkusr *disco){
     cout << "\n     ----COMANDO MKUSR----\n"<<endl;
    ...
    CrearUsuario(super, contenido_usuarios);

    cout << "PROCESO EXITOSAMENTE!!" <<endl;

}
void obmkusr::CrearUsuario(structs::SuperBloque super, string contenido){
    ...
    for(structs::LOGIN e: lista_USER){
        //cout << "GRUPO: "<<e.usuario <<" posicion" << e.posicion<<endl;
    ...
    contenido += to_string(no)+",U,"+this->grupo+","+this->name+","+this->password+"\n";
    obedit *edit = new obedit();
    edit->RUTA = RUTA_PARTITTION;
    edit->EMPIEZA = PART_START;
    edit->Modificar(super, 1, contenido);
}
string obmkusr::EliminarComillas(string cadena){
    string ruta;
    if(cadena[0] == '"'){
...
void obmkusr::getSuperBloque(structs::SuperBloque *sp, string ruta, int part_Start){
...
bool obmkusr::getInodo(structs::TabladeInodos*sp, string ruta, int part_Start){
...
bool obmkusr::getBlock_Files(structs::BloquesdeArchivos *sp, string ruta, int part_Start){
...
bool obmkusr::getBlock_Carpets(structs::BloquesdeCarpetas *sp, string ruta, int part_Start){
...
bool obmkusr::getBlock_Apuntador(structs::BloquesdeApuntadores *sp, string ruta, int part_Start){
...
string obmkusr::NombreArchivo(string path){
    int contador = 0;
    string cadena = "";
...
bool obmkusr::existelaparticionmontada(structs::DiscosMontados tmp[], string codigo, string *ruta, string *nombre, int *start_particion, int *size_particion){
    for(int i = 0; i < 26; i++){
        if(tmp[i].status == 1){
            for(int j = 0; j < 99; j++){
...
bool obmkusr::ExisteMBR(string ruta, structs::MBR *mbr){
    structs::MBR tmp_disco;
    FILE *file = fopen(ruta.c_str(), "rb+");
    if (file != NULL){
...
bool obmkusr::ExisteNombre(structs::MBR tmp_MBR, string name, int *Ext, int *start_part, int *size_part){
    *Ext = 0;
...
bool obmkusr::ExisteNombreLogica(string ruta, string name, int part_start_ext, int *start_part, int *size_part){
