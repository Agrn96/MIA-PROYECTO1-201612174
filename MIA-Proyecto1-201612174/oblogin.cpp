...
void oblogin::mostrardatos(oblogin *disco){

     cout << "\n     ----COMANDO LOGIN----\n"<<endl;
    string id=this->id_particion;
    ...
    if(USUARIO_ONLINE.usuario != "-1"){
        cout << "YA EXISTE UN USUARIO EN LINEA "<< endl;
        return;
    }
    ...
    list<structs::LOGIN> lista_usuarios;
    ...
    while ((pos = s.find(delimiter)) != string::npos) {
        ...
        //SI ES USUARIO GUARDAMOS LA INFORMACION COMO USUARIO
        if(!EsGrupo){
            structs::LOGIN log_aux;
            log_aux.posicion = aux[0];
            log_aux.tipo = aux[1];
            log_aux.grupo = aux[2];
            log_aux.usuario = aux[3];
            log_aux.password = entrada2;
            lista_usuarios.push_back(log_aux);

        }
        s.erase(0, pos + delimiter.length());
    }

    //-----CONFIRMAMOS SI LOS USUARIOS COINCIDEN Y DAR ACCESO
    bool COINCIDEN = false;
    for(structs::LOGIN log_aux: lista_usuarios){
        //cout << "USUARIO: "<< log_aux.usuario << " -- "<<sizeof (log_aux.usuario) << endl;
        //cout << "PASSWORD: "<< log_aux.password << " -- "<<sizeof (log_aux.password) << endl;
        if(log_aux.usuario == this->usuario && log_aux.posicion != "0"){
            //cout << "coinciden u"<< endl;
            if(log_aux.password == this->pasword){
                COINCIDEN = true;
                string no_grupo = RecuperarGrupo(log_aux.grupo, contenido);
                USUARIO_ONLINE.no_grupo = no_grupo;
                USUARIO_ONLINE.posicion = log_aux.posicion;
                if(log_aux.usuario == "root"){
                    USUARIO_ONLINE.grupo = "root";
                    USUARIO_ONLINE.no_grupo = "1";
                    USUARIO_ONLINE.posicion = "1";
                }
                break;
            }
        }
    ...
    //cout << "NO_GRUPO: "<< USUARIO_ONLINE.no_grupo << endl;

    if(COINCIDEN){
        USUARIO_ONLINE.usuario = this->usuario;
        USUARIO_ONLINE.password = this->pasword;
        USUARIO_ONLINE.id_particion = this->id_particion;
        cout << "     ACCESO EXITOSO!!"<<endl;
        cout << "ID PARTICION: "<<this->id_particion << " A UTILIZAR!" <<endl;

    }
    else{
        cout << "         ACCESO DENEGADO"<<endl;
        cout << "LA CONTRASEÑA O USUARIO NO COINCIDEN!!"<<endl;
        return;
    }
}
void oblogin::logout(){
    if(USUARIO_ONLINE.usuario == "-1"){
        cout << "ERROR: NO HAY UN USUARIO EN LINEA!!" << endl;
    }
    else{
        USUARIO_ONLINE.usuario = "-1";
        USUARIO_ONLINE.password = "-1";
        USUARIO_ONLINE.id_particion = "-1";
        cout << "SE CERRO SESION CORRECTAMENTE!!"<<endl;
    }
    return;
}
string oblogin::RecuperarGrupo(string grupo,string contenido){
    ...
    for(structs::GROUP e: lista_GROUP){
        if(e.nombre == grupo && e.posicion != "0"){
            return e.posicion;
        }
    }
...
string oblogin::EliminarComillas(string cadena){
    string ruta;
    if(cadena[0] == '"'){
...
void oblogin::getSuperBloque(structs::SuperBloque *sp, string ruta, int part_Start){
...
bool oblogin::getInodo(structs::TabladeInodos*sp, string ruta, int part_Start){
...
bool oblogin::getBlock_Files(structs::BloquesdeArchivos *sp, string ruta, int part_Start){
...
bool oblogin::getBlock_Carpets(structs::BloquesdeCarpetas *sp, string ruta, int part_Start){
...
bool oblogin::getBlock_Apuntador(structs::BloquesdeApuntadores *sp, string ruta, int part_Start){
...
string oblogin::NombreArchivo(string path){
    int contador = 0;
    string cadena = "";
...
bool oblogin::existelaparticionmontada(structs::DiscosMontados tmp[], string codigo, string *ruta, string *nombre, int *start_particion, int *size_particion){
    for(int i = 0; i < 26; i++){
        if(tmp[i].status == 1){
            for(int j = 0; j < 99; j++){
...
bool oblogin::ExisteMBR(string ruta, structs::MBR *mbr){
    structs::MBR tmp_disco;
    FILE *file = fopen(ruta.c_str(), "rb+");
    if (file != NULL){
...
bool oblogin::ExisteNombre(structs::MBR tmp_MBR, string name, int *Ext, int *start_part, int *size_part){
    *Ext = 0;
...
bool oblogin::ExisteNombreLogica(string ruta, string name, int part_start_ext, int *start_part, int *size_part){
