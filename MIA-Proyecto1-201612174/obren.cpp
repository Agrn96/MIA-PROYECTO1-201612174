...
void obren::mostrardatos(obren *disco){
   ...
   //OBTENER ATRIBUTOS DEL DISCO MONTADO
    ...
    if(Encontrelacarpeta){
        ...
        int contador_i = 0;
        for(int j = 0; j < 4; j++){
            string nombre_a(BloqueCarpeta_Actual.b_content[j].b_name );
            if(nombre_a == disco->name){
               cout << "YA EXISTE UN ARCHIVO O CARPETA CON EL NOMBRE "<< NombreAr << endl;
               return;
            }
        }
        ...
        for(int j = 0; j < 4; j++){
            string nombre_a(BloqueCarpeta_Actual.b_content[j].b_name );
            if(NombreAr == nombre_a){
                strcpy(BloqueCarpeta_Actual.b_content[j].b_name, disco->name.c_str());
                ...
                fwrite(&BloqueCarpeta_Actual, sizeof(structs::BloquesdeArchivos), 1, file);
                fclose(file);
...
string obren::EliminarComillas(string cadena){
    string ruta;
    if(cadena[0] == '"'){
...
void obren::getSuperBloque(structs::SuperBloque *sp, string ruta, int part_Start){
...
bool obren::getInodo(structs::TabladeInodos*sp, string ruta, int part_Start){
...
bool obren::getBlock_Files(structs::BloquesdeArchivos *sp, string ruta, int part_Start){
...
bool obren::getBlock_Carpets(structs::BloquesdeCarpetas *sp, string ruta, int part_Start){
...
bool obren::getBlock_Apuntador(structs::BloquesdeApuntadores *sp, string ruta, int part_Start){
...
string obren::NombreArchivo(string path){
    int contador = 0;
    string cadena = "";
...
bool obren::existelaparticionmontada(structs::DiscosMontados tmp[], string codigo, string *ruta, string *nombre, int *start_particion, int *size_particion){
    for(int i = 0; i < 26; i++){
        if(tmp[i].status == 1){
            for(int j = 0; j < 99; j++){
...
bool obren::ExisteMBR(string ruta, structs::MBR *mbr){
    structs::MBR tmp_disco;
    FILE *file = fopen(ruta.c_str(), "rb+");
    if (file != NULL){
...
bool obren::ExisteNombre(structs::MBR tmp_MBR, string name, int *Ext, int *start_part, int *size_part){
    *Ext = 0;
...
bool obren::ExisteNombreLogica(string ruta, string name, int part_start_ext, int *start_part, int *size_part){
