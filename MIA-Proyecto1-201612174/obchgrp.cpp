...
void obchgrp::mostrardatos(obchgrp *disco){
     cout << "\n     ----COMANDO CHGRP----\n"<<endl;
    this->name = EliminarComillas(this->name);
    this->password = EliminarComillas(this->password);
    this->grupo = EliminarComillas(this->grupo);

    if(this->name.length() > 10){
        cout << "EL USUARIO NO PUEDE TENER MAS DE 10 CARACTERES"<<endl;
        return;
    }
    ...
    if(USUARIO_ONLINE.usuario != "root"){
        cout << "SOLO EL USUARIO root1 PUEDE AGREGAR USUARIOS"<< endl;
        return;
    }
    ...
    string contenido_usuarios = contenido;
    ...
    for(structs::GROUP e: lista_GROUP){
        ...
        if(e.nombre == this->grupo && e.posicion != "0"){
            ExisteGrupo = true;
            break;
        }
    }
    ...
    if(!ExisteGrupo){
        cout << "EL GRUPO INGRESADO NO EXISTE!!"<<endl;
        return;
    }

    ModificarUsuario(super, contenido_usuarios);

    cout << "PROCESO EXITOSAMENTE!!" <<endl;

}
void obchgrp::ModificarUsuario(structs::SuperBloque super, string contenido){
    ...
    for(structs::LOGIN e: lista_USER){
        //cout << "user: "<< e.usuario << " posicion: "<< e.posicion<<endl;
        if(e.usuario == this->name && e.posicion != "0"){
            grupo_eliminar = e.posicion+",U,"+e.grupo+","+e.usuario+","+e.password+"\n";
            grupo_nuevo = e.posicion+",U,"+this->grupo+","+e.usuario+","+e.password+"\n";
            ExisteElGrupo = true;
        }
    }
...
string obchgrp::EliminarComillas(string cadena){
    string ruta;
    if(cadena[0] == '"'){
...
void obchgrp::getSuperBloque(structs::SuperBloque *sp, string ruta, int part_Start){
...
bool obchgrp::getInodo(structs::TabladeInodos*sp, string ruta, int part_Start){
...
bool obchgrp::getBlock_Files(structs::BloquesdeArchivos *sp, string ruta, int part_Start){
...
bool obchgrp::getBlock_Carpets(structs::BloquesdeCarpetas *sp, string ruta, int part_Start){
...
bool obchgrp::getBlock_Apuntador(structs::BloquesdeApuntadores *sp, string ruta, int part_Start){
...
string obchgrp::NombreArchivo(string path){
    int contador = 0;
    string cadena = "";
...
bool obchgrp::existelaparticionmontada(structs::DiscosMontados tmp[], string codigo, string *ruta, string *nombre, int *start_particion, int *size_particion){
    for(int i = 0; i < 26; i++){
        if(tmp[i].status == 1){
            for(int j = 0; j < 99; j++){
...
bool obchgrp::ExisteMBR(string ruta, structs::MBR *mbr){
    structs::MBR tmp_disco;
    FILE *file = fopen(ruta.c_str(), "rb+");
    if (file != NULL){
...
bool obchgrp::ExisteNombre(structs::MBR tmp_MBR, string name, int *Ext, int *start_part, int *size_part){
    *Ext = 0;
...
bool obchgrp::ExisteNombreLogica(string ruta, string name, int part_start_ext, int *start_part, int *size_part){
