...
void obrmgrp::mostrardatos(obrmgrp *disco){
     cout << "\n     ----COMANDO RMGRP----\n"<<endl;
    ...
    if(USUARIO_ONLINE.usuario != "root"){
        cout << "SOLO EL USUARIO root PUEDE ELIMINAR GRUPOS"<< endl;
        return;
    }
    ...
    while ((pos = s.find(delimiter)) != string::npos) {
        ...
        if(EsGrupo){
            structs::GROUP grupo;
            //cout << "POSICION "<<aux[0]<< " NOMBRE: "<< entrada2 << endl;
    ...
    for(structs::GROUP e: lista_GROUP){
        if(e.nombre == this->name && e.posicion != "0"){
            grupo_eliminar = e.posicion+",G,"+e.nombre+"\n";
            grupo_nuevo = "0,G,"+e.nombre+"\n";
            ExisteElGrupo = true;
        }
    }
...
string obrmgrp::EliminarComillas(string cadena){
    string ruta;
    if(cadena[0] == '"'){
...
void obrmgrp::getSuperBloque(structs::SuperBloque *sp, string ruta, int part_Start){
...
bool obrmgrp::getInodo(structs::TabladeInodos*sp, string ruta, int part_Start){
...
bool obrmgrp::getBlock_Files(structs::BloquesdeArchivos *sp, string ruta, int part_Start){
...
bool obrmgrp::getBlock_Carpets(structs::BloquesdeCarpetas *sp, string ruta, int part_Start){
...
bool obrmgrp::getBlock_Apuntador(structs::BloquesdeApuntadores *sp, string ruta, int part_Start){
...
string obrmgrp::NombreArchivo(string path){
    int contador = 0;
    string cadena = "";
...
bool obrmgrp::existelaparticionmontada(structs::DiscosMontados tmp[], string codigo, string *ruta, string *nombre, int *start_particion, int *size_particion){
    for(int i = 0; i < 26; i++){
        if(tmp[i].status == 1){
            for(int j = 0; j < 99; j++){
...
bool obrmgrp::ExisteMBR(string ruta, structs::MBR *mbr){
    structs::MBR tmp_disco;
    FILE *file = fopen(ruta.c_str(), "rb+");
    if (file != NULL){
...
bool obrmgrp::ExisteNombre(structs::MBR tmp_MBR, string name, int *Ext, int *start_part, int *size_part){
    *Ext = 0;
...
bool obrmgrp::ExisteNombreLogica(string ruta, string name, int part_start_ext, int *start_part, int *size_part){
